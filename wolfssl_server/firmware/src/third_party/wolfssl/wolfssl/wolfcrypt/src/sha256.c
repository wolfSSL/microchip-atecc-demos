...
/* Hardware Acceleration */
    ...
    */
...
#elif defined(WOLFSSL_ATECC_SHA256) && !defined(WOLFSSL_PIC32MZ_HASH)
    ...
    int wc_InitSha256_ex(wc_Sha256* sha256, void* heap, int devId)
    {
        ...
        ret = wolfSSL_CryptHwMutexLock();
        if (ret == 0) {
            ret = atcab_sha_base(SHA_MODE_SHA256_START, 0, NULL, NULL, NULL);
            if (ret == ATCA_SUCCESS) {
                sha256->ctxSz = SHA_CONTEXT_MAX_SIZE;
                ret = atcab_sha_read_context(sha256->ctx, &sha256->ctxSz);
            }
            wolfSSL_CryptHwMutexUnLock();
        }
    ...
    int wc_Sha256Update(wc_Sha256* sha256, const byte* data, word32 len)
    {
        ...
        ret = wolfSSL_CryptHwMutexLock();
        if (ret == 0) {
            ret = atcab_sha_write_context(sha256->ctx, sha256->ctxSz);
            if (ret == ATCA_SUCCESS) {
                while (remaining > 0) {
                    copySz = remaining < ATCA_SHA256_BLOCK_SIZE ?
                                 remaining : ATCA_SHA256_BLOCK_SIZE;

                    /* can ony process up to ATCA_SHA256_BLOCK_SIZE at once */
                    ret = atcab_sha_base(SHA_MODE_SHA256_UPDATE, copySz, idx,
                                         NULL, NULL);
                    if (ret == ATCA_SUCCESS) {
                        remaining -= ATCA_SHA256_BLOCK_SIZE;
                        idx += copySz;
    ...
    {
        int ret = 0;
        uint16_t outSz = WC_SHA256_DIGEST_SIZE;
        ...
        if (ret == 0) {
            ...
            if (ret == ATCA_SUCCESS) {
                ret = atcab_sha_base(
                          SHA_MODE_SHA256_END | SHA_MODE_TARGET_OUT_ONLY,
                          0, NULL, hash, &outSz);
            }
            wolfSSL_CryptHwMutexUnLock();
        }
        ...
        if (ret == 0) {
            ret = wc_InitSha256(sha256); /* reset state */
        }
